...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.specialSection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.special-sections.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('section_place') ? 'is-invalid' : '' }}" name="section_place" id="section_place">
                    ...
                    @foreach(App\Models\SpecialSection::SECTION_PLACE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('section_place', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
